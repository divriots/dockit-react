...
const Box = ({ className, showcaseStyle, showcaseClass, showcase, index }) => (<div
  style={{
    height: '6rem',
    minWidth: '8rem',
...
>
  <Caption text={showcase} style={{ color: '#fff' }} />
</div>
);

const StyledBox = styled(Box)`${position}`;

export const ZIndexShowcases = ({ values, classes, isStyledComponent = false }: { values?: string[], classes?: string[], isStyledComponent?: boolean }) => {

  let { getStyle, getClass, showcases } = values
    ? { getStyle: val => ({ zIndex: val }), getClass: () => '', showcases: values }
